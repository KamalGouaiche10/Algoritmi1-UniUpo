...
extern const void* upo_bst_check_successor(const upo_bst_t, const void*);
...
int main()
{
    ...
    for (i = 1; i <= n; ++i)
    {
        ...
        printf("Test BST Successor #%lu => %s\n", i, (ret == TEST_OK) ? "[OK]" : "[FAIL]");
    }

    ret = test_empty_bst();
    printf("Test Empty BST Successor => %s\n", (ret == TEST_OK) ? "[OK]" : "[FAIL]");
...
int test_bst(void *keys, size_t key_sz, void *values, size_t value_sz, size_t n, void *no_keys, size_t m, upo_bst_comparator_t key_cmp, char* (*key_to_string)(const void*), char* (*value_to_string)(const void*))
{
    ...
    if (n > 0)
    {
        ...
        // Find the successor of each key

        // Keys
        for (i = 0; i < n && ret == TEST_OK; ++i)
        {
            void *pkey = NULL;
            const void *p_succ = NULL;
            const void *p_succ_check = NULL;
            ...
            p_succ = upo_bst_successor(bst, pkey);

            p_succ_check = upo_bst_check_successor(bst, pkey);

            if (p_succ == NULL && p_succ_check != NULL)
            {
                char *key_str = key_to_string(pkey);
                char *succ_check_str = key_to_string(p_succ_check);
                fprintf(stderr, "[file: %s, line: %d] ERROR: Input key: '%s' -> Expected successor '%s', got none.\n", __FILE__, __LINE__, key_str, succ_check_str);
                free(succ_check_str);
                free(key_str);
            ...
            else if (p_succ != NULL && p_succ_check == NULL)
            {
                char *key_str = key_to_string(pkey);
                char *succ_str = key_to_string(p_succ);
                fprintf(stderr, "[file: %s, line: %d] ERROR: Input key: '%s' -> Expected no successor, got '%s'.\n", __FILE__, __LINE__, key_str, succ_str);
                free(succ_str);
                free(key_str);
                fprintf(stderr, "[file: %s, line: %d]: <BST>\n", __FILE__, __LINE__);
                upo_bst_dump(bst, stderr, key_to_string, 1, value_to_string, 1);
            ...
            else if (p_succ != p_succ_check && memcmp(p_succ, p_succ_check, key_sz))
            {
                char *key_str = key_to_string(pkey);
                char *succ_str = key_to_string(p_succ);
                char *succ_check_str = key_to_string(p_succ_check);
                fprintf(stderr, "[file: %s, line: %d] ERROR: Input key: '%s' -> Expected successor '%s', got '%s'.\n", __FILE__, __LINE__, key_str, succ_check_str, succ_str);
                free(succ_check_str);
                free(succ_str);
                free(key_str);
                fprintf(stderr, "[file: %s, line: %d]: <BST>\n", __FILE__, __LINE__);
        ...
        // Find the successor of each no-key
    ...
    else
    {
        ...
        p_succ = upo_bst_successor(bst, &dummy_key);

        if (p_succ != NULL)
        {
            char *succ_str = key_to_string(p_succ);
            fprintf(stderr, "[file: %s, line: %d] ERROR: Empty BST -> Expected no successor, got '%s'.\n", __FILE__, __LINE__, succ_str);
            free(succ_str);
            ret = TEST_FAIL;
        }
    }
